...
void AuthSession::RealmListCallback(PreparedQueryResult result)
{
    ...
    for (RealmList::RealmMap::value_type const& i : sRealmList->GetRealms())
    {
        ...
        bool okBuild = ((_expversion & POST_BC_EXP_FLAG) && (realm.Build == _build||12340 == _build || 13930 == _build)) || ((_expversion & PRE_BC_EXP_FLAG) && !AuthHelper::IsPreBCAcceptedClientBuild(realm.Build));
