...
namespace
{
    ...
    public:
		...
		struct CS_TransmogrificationAI : public ScriptedAI
		{
			CS_TransmogrificationAI(Creature* creature) : ScriptedAI(creature) { }
			...
			std::stack<uint32> stackWindow;
			void BackMenu(Player* player)
			{
				stackWindow.pop();
				uint32 gossipListId = stackWindow.top();
				stackWindow.pop();
				GossipSelect(player, 0, gossipListId);
			}
			bool GossipSelect(Player* player, uint32 /*menuId*/, uint32 gossipListId) override
			{
				stackWindow.push(gossipListId);
				...
				case EQUIPMENT_SLOT_END: // Show items you can use
					ShowTransmogItems(player, me, action);
					break;
		...
		{
			return new CS_TransmogrificationAI(creature);
		}
    };
